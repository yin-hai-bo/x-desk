...
- **Video Wallpaper**: A video file or browser-playable video URL rendered behind desktop icons as desktop background content.
...
- `x-desk-webview` renders video files and browser-playable video URLs through WebView2. It still has an internal HTML entry point for non-config callers, but config input is treated as video source only.
